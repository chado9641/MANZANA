...
class ListaPersonaje{ //<-- Permite usar variables y metodos de ListaEspecie en ListaPersonaje
    ...
    void mostrarEspecies2(ListaEspecie listaesp){ //<-- Muestra especies disponibles para crear al personaje
        listaesp.mostrarEspecies();
    }

    int indicarEspeciePersonaje(ListaEspecie listaesp){ //Indica la especie del personaje
        
        listaesp.mostrarEspecies();

        cout << "Indique el id de la especie deseada" << endl;
        int idEspecie;
        cin >> idEspecie;
        cin.ignore();
        return idEspecie;
    ...
    void modificarPersonaje(ListaEspecie listaesp){
        ...
        int idEspecie=indicarEspeciePersonaje(listaesp); //Pedira el id de la especie deseada

       /* if(idEspecie<1 || idEspecie>cantidad){ //Evita id que no existe

            cout << "id invalida";
            return;
        } */

        Especie* buscado ; //<-- arreglar a que apunte a cabeza de especie
...
class ListaSalas{
    ...
    void modificarSala(){
        ...
        cout << "Distancia: ";
        cin >> actual->distancia; 

        actual->cantidadOrcos=0;
        actual->numeroDeSala=cantidad;
